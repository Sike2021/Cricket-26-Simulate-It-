...
import { aggregateStats, calculatePlayerRankings, PlayerRanking } from '../utils';
...
const Stats: React.FC<StatsProps> = ({ gameData, viewPlayerProfile }) => {
    const [statType, setStatType] = useState<'batting' | 'bowling' | 'milestones' | 'rankings'>('batting');
    ...
    const rankings = useMemo(() => {
        if (statType !== 'rankings' || !Object.values(Format).includes(selectedFormatOption as Format)) return null;
        return calculatePlayerRankings(gameData.allPlayers, selectedFormatOption as Format, gameData.teams);
    }, [gameData.allPlayers, selectedFormatOption, statType]);
    ...
    const handleStatTypeChange = (type: 'batting' | 'bowling' | 'milestones' | 'rankings') => {
        setStatType(type);
        if (type === 'batting') {
    ...
    const sortedPlayers = useMemo(() => {
        if (statType === 'milestones' || statType === 'rankings') return [];
    ...
    const RankingTable = ({ title, data }: { title: string, data: PlayerRanking[] }) => (
        <div className="bg-white dark:bg-gray-800/50 rounded-lg shadow-md overflow-hidden">
            <h3 className="bg-slate-200 dark:bg-slate-700 p-2 font-bold text-center text-sm uppercase tracking-wider">{title}</h3>
            <table className="w-full text-sm">
                ...
                <tbody>
                    {data.slice(0, 10).map((item, index) => (
                        <tr key={item.player.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            <td className="p-2 text-center font-bold text-gray-400">
                                {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : index + 1}
                            </td>
                            <td className="p-2">
                                <div className="font-semibold">{item.player.name}</div>
                                <div className="text-[10px] text-gray-500">{item.teamName}</div>
                            </td>
                            <td className="p-2 text-right font-bold text-teal-600 dark:text-teal-400">{item.points}</td>
                        </tr>
                    ))}
                    {data.length === 0 && <tr><td colSpan={3} className="p-4 text-center text-gray-500">Not enough data</td></tr>}
                </tbody>
            </table>
        </div>
    );
    ...
    return (
        <div className="p-2 h-[calc(100vh-90px)] flex flex-col">
            ...
            <div className="flex justify-center border-b border-gray-300 dark:border-gray-700 mb-2">
                <button onClick={() => handleStatTypeChange('batting')} className={`px-3 py-2 text-xs font-semibold ${statType === 'batting' ? 'border-b-2 border-teal-500 text-teal-500' : ''}`}>Batting</button>
                <button onClick={() => handleStatTypeChange('bowling')} className={`px-3 py-2 text-xs font-semibold ${statType === 'bowling' ? 'border-b-2 border-teal-500 text-teal-500' : ''}`}>Bowling</button>
                <button onClick={() => handleStatTypeChange('rankings')} className={`px-3 py-2 text-xs font-semibold ${statType === 'rankings' ? 'border-b-2 border-teal-500 text-teal-500' : ''}`}>Rankings</button>
                <button onClick={() => handleStatTypeChange('milestones')} className={`px-3 py-2 text-xs font-semibold ${statType === 'milestones' ? 'border-b-2 border-teal-500 text-teal-500' : ''}`}>Milestones</button>
            </div>
                ...
                {statType === 'rankings' ? (
                    rankings ? (
                        <div className="space-y-6 pb-4">
                            <div className="text-center text-xs text-gray-500 mb-2 italic">Rankings are calculated based on runs, wickets, consistency, and impact.</div>
                            <RankingTable title="Top Batters" data={rankings.batters} />
                            <RankingTable title="Top Bowlers" data={rankings.bowlers} />
                            <RankingTable title="Top All-Rounders" data={rankings.allRounders} />
                        </div>
                    ) : (
                        <div className="p-4 text-center text-gray-500">
                            Rankings are only available for specific tournament formats, not aggregate views. Please select a specific tournament from the dropdown.
                        </div>
                    )
                ) : statType !== 'milestones' ? (
                <table className="w-full text-sm">
