                ...
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {standings.map((s, index) => {
                            const logo = gameData.allTeamsData.find(t => t.id === s.teamId)?.logo;
                            return (
                                <tr key={s.teamId} className={`border-b dark:border-gray-700/50 ${index < 4 ? 'bg-teal-500/10' : ''}`}>
                                    <td className="p-2 font-semibold flex items-center gap-2">
                                        {logo && <div className="w-5 h-5" dangerouslySetInnerHTML={{__html: logo}}></div>}
                                        {s.teamName}
                                    </td>
                            ...
                            >
                                ...
                                <div className="flex items-center justify-center gap-2 font-semibold">
                                    <div className="flex items-center gap-1">
                                        {logoA && <div className="w-4 h-4" dangerouslySetInnerHTML={{__html: logoA}}></div>}
                                        <span>{resolved.teamA}</span>
                                    </div>
                                    <span className="mx-1 text-xs text-gray-500">vs</span>
                                    <div className="flex items-center gap-1">
                                        {logoB && <div className="w-4 h-4" dangerouslySetInnerHTML={{__html: logoB}}></div>}
